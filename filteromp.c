...
int main(int argc, char** argv) {
	int i, myid, numprocs, dim, *sendcounts, *displs, RGB, change, glob_conv,rep=10000;
	if (argc != 6 && argc != 5) {
		fprintf(stderr,
		"Not correct arguments. Please give me 1) the path of the image file, 2) the width, 3) the height 4)0 if not RGB 5)repetitions if not convergence .\n");
		exit(EXIT_FAILURE);
	}
	char *path = argv[1];
	...
	unsigned int height = atoi(argv[3]);	//Get the array that represends the file
	//Arguments things
	...
	if (!RGB){
		...
		if (myid == 0){
			image_array = getGreyImageArray(path, height, width);
			if (image_array ==NULL){
				exit(EXIT_FAILURE);
			      MPI_Finalize();
				exit(EXIT_FAILURE);
		...
		for (i=0;i<rep;i++){
			send_pixels(myid, numprocs, locbuf[choose], width, height, dim, send_req,1);

			recv_pixels(myid, numprocs, &ul, &ur, &dr, &dl, up, left, right, down, width, height, dim, recv_req,1);

			apply_inner_filter(locbuf, width/dim, height/dim, choose);

			MPI_Waitall(8, recv_req, recv_status);

			apply_outer_filter(locbuf, width/dim, height/dim, choose,up,down, left, right, ul,ur,dl,dr);

			MPI_Waitall(8, send_req, send_status);
			choose=1-choose;
			if (rep==10000){
				if (memcmp(locbuf[0], locbuf[1], height * width/numprocs) )
					change = 1;
				else
					change = 0;
...
void apply_inner_filter(unsigned char** locbuf,const unsigned int width, const unsigned int height,unsigned char choose){
	unsigned int i;
